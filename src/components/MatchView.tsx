...
export default function MatchView({ group, match, players, members, announcements, isManager }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {isManager && (
        <Link to={`/manage/match/${match.id}`} className="block rounded-2xl bg-neutral-900 px-4 py-4 text-center font-semibold text-white">
  ...
  return (
    <Card className="bg-pitch-900 text-white">
      <div className="mb-1 flex items-center justify-between gap-2">
        <span className="text-xs font-bold uppercase tracking-wider text-green-300">{group.name}</span>
        <StatusPill status={status} />
      </div>
      <h1 className="text-2xl font-extrabold leading-tight">{formatDateLong(match.date)}</h1>
      <div className="mt-1 text-lg font-semibold text-green-200">{formatTimeRange(match.startTime, match.durationMinutes)}</div>
      <div className="mt-2 text-sm text-neutral-200">
        {match.venueName || 'Local a definir'}
        {match.courtName && <span> · {match.courtName}</span>}
      </div>
      {match.address && (
        <a className="mt-0.5 block text-xs text-neutral-400 underline-offset-2 hover:underline" href={`https://maps.google.com/?q=${encodeURIComponent(match.address)}`} target="_blank" rel="noreferrer">
          {match.address}
        </a>
      )}
      {(status === 'open' || status === 'confirmed') && (
        <div className="mt-3 rounded-xl bg-white/10 px-3 py-2 text-sm font-semibold">
    ...
    <Card>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          ...
          className={`rounded-2xl py-5 text-lg font-extrabold transition ${me?.status === 'available' ? 'bg-green-600 text-white shadow-lg ring-4 ring-green-600/30' : 'bg-green-50 text-green-800 hover:bg-green-100'} disabled:opacity-60`}
        ...
        <button
          ...
          className={`rounded-2xl py-5 text-lg font-extrabold transition ${me?.status === 'unavailable' ? 'bg-red-600 text-white shadow-lg ring-4 ring-red-600/30' : 'bg-red-50 text-red-800 hover:bg-red-100'} disabled:opacity-60`}
      ...
      {me?.status === 'available' && (
        <div className="mt-4 fade-in">
          <div className="mb-2 text-sm font-semibold text-neutral-700">Sua posição</div>
          <Toggle options={positions} value={me.position ?? 'line'} onChange={(v) => answer('available', v as Position)} disabled={busy} />
          {me.position === 'goalkeeper' && <p className="mt-2 text-xs text-neutral-500">Goleiros não pagam a quadra.</p>}
        </div>
      )}
      {!me && <p className="mt-3 text-center text-xs text-neutral-500">Você pode mudar sua resposta enquanto a partida estiver aberta.</p>}
    </Card>
  )
}
...
function PaymentCard({ group, match, me, split }: { group: Group; match: Match; me: MatchPlayer; split: ReturnType<typeof computeSplit> }) {
  ...
  return (
    <Card>
      ...
      {isGk ? (
        <p className="text-sm text-neutral-600">Goleiros não entram no rateio. Obrigado por defender o gol!</p>
      ) : (
        <>
          <div className="flex items-end justify-between">
            <div>
              <div className="text-xs text-neutral-500">Sua parte ({split.payers.length} pagante{split.payers.length === 1 ? '' : 's'} · quadra {formatMoney(split.cost)})</div>
          ...
          {hasPix ? (
            <div className="mt-3 space-y-2">
              <div className="rounded-xl bg-neutral-50 px-3 py-2">
                <div className="text-[11px] font-semibold uppercase text-neutral-500">PIX ({group.pixKeyType})</div>
            ...
            <p className="mt-2 text-xs text-neutral-500">O organizador ainda não cadastrou a chave PIX.</p>
          )}
        </>
      )}
    </Card>
...
function TeamsCard({ match, players }: { match: Match; players: MatchPlayer[] }) {
  ...
  return (
    <Card>
      <SectionTitle right={match.teamsGeneratedAt ? <span className="text-xs text-neutral-400">{formatDateTime(match.teamsGeneratedAt)}</span> : null}>Times</SectionTitle>
      <div className="grid grid-cols-2 gap-3">
        {match.teams.map((t, i) => (
          <div key={t.id} className={`rounded-xl p-3 ${['bg-blue-50', 'bg-amber-50', 'bg-emerald-50', 'bg-rose-50', 'bg-violet-50', 'bg-cyan-50'][i % 6]}`}>
            ...
            <ul className="space-y-0.5 text-sm">
              ...
              {t.playerIds.length === 0 && <li className="text-neutral-400">vazio</li>}
            </ul>
          </div>
        ))}
      </div>
    ...
    <Card>
      <div className="space-y-4">
        <div>
          ...
          {ordered.length === 0 && <p className="text-sm text-neutral-500">Ninguém confirmou ainda.</p>}
          <ul className="divide-y divide-neutral-100">
            {ordered.map((p) => (
              <li key={p.id} className="flex items-center justify-between py-2">
                <div className="flex items-center gap-2">
                  <span className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${p.position === 'goalkeeper' ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-800'}`}>
                    {p.position === 'goalkeeper' ? 'G' : 'L'}
                  </span>
                  <span className="font-medium">{shortName(p.name)}</span>
                  <span className="text-xs text-neutral-500">{p.position === 'goalkeeper' ? 'Goleiro' : 'Linha'}</span>
                </div>
                {p.position === 'goalkeeper' ? (
                  ...
                  <button type="button" onClick={() => togglePaid(p)} aria-label={`${p.paid ? 'Marcar como não pago' : 'Marcar como pago'}: ${p.name}`}
                    className={`rounded-full px-2.5 py-1 text-xs font-bold ${p.paid ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                    {p.paid ? '🟢 PAGO' : '🔴 NÃO PAGO'}
                  </button>
                ) : (
                  <span className={`text-xs font-bold ${p.paid ? 'text-green-700' : 'text-red-600'}`}>{p.paid ? '🟢 Pago' : '🔴 Não pago'}</span>
                )}
              </li>
            ))}
          </ul>
        ...
        <div>
          ...
          {split.unavailable.length === 0 ? <p className="text-sm text-neutral-500">—</p> : (
            <ul className="flex flex-wrap gap-2">
              {split.unavailable.map((p) => <li key={p.id} className="rounded-full bg-neutral-100 px-3 py-1 text-sm text-neutral-600 line-through decoration-neutral-400">{shortName(p.name)}</li>)}
            </ul>
          )}
        </div>
        ...
        {open && (
          <div>
            ...
            {pending.length === 0 ? <p className="text-sm text-neutral-500">Todos responderam 🎉</p> : (
              <ul className="flex flex-wrap gap-2">
                {pending.map((m) => <li key={m.uid} className="rounded-full bg-neutral-100 px-3 py-1 text-sm text-neutral-600">{shortName(m.name)}</li>)}
              </ul>
            )}
          </div>
        )}
  ...
  return (
    <div className="space-y-2">
      {items.slice(0, 3).map((a) => (
        <div key={a.id} className="fade-in rounded-2xl border-l-4 border-amber-500 bg-amber-50 px-4 py-3">
          <div className="text-[11px] font-bold uppercase tracking-wide text-amber-700">📣 Aviso do organizador · {formatDateTime(a.createdAt)}</div>
          <p className="mt-0.5 whitespace-pre-wrap text-sm text-amber-950">{a.text}</p>
        </div>
      ))}
    </div>
  )
